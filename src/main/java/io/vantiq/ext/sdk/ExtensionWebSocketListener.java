...
public class ExtensionWebSocketListener extends WebSocketListener {
    ...
    private void doOnMessage(byte[] data) {
        if (this.isClosed) {
        ...
        if (msg.get("op") == null) {
            ...
            if (client.isAuthed()) {
                ...
                synchronized (client) {
                    ...
                    if (message.getStatus() == 200 && !client.isAuthed()) {
     ...
     */
    @Override
    public void onFailure(WebSocket webSocket, Throwable e, okhttp3.Response response) {
